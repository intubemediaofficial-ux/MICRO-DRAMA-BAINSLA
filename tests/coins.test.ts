...
describe("coin economy", () => {
  beforeAll(async () => {
    const user = await prisma.user.create({
      data: { email: `coin-${suffix}@test.local`, referralCode: `COIN${suffix.slice(0, 8)}` },
    });
    userId = user.id;
    ...
    freeEpisodeId = episodes[0].id;
    episodeId = episodes[1].id;
  });
  afterAll(async () => {
    await prisma.user.delete({ where: { id: userId } });
    await prisma.series.deleteMany({ where: { slug: `coin-${suffix}` } });
    await prisma.$disconnect();
  });
  it("rejects insufficient debit and leaves the ledger invariant intact", async () => {
    await expect(
      import("../src/server/coins").then(({ debit }) => debit(userId, 21, "EPISODE_UNLOCK")),
    ).rejects.toBeInstanceOf(InsufficientCoins);
  ...
  it("charges concurrent unlocks once", async () => {
    ...
    expect(user.coinBalance).toBe(10);
  });
  it("makes daily check-in idempotent", async () => {
    const first = await dailyCheckin(userId);
    const second = await dailyCheckin(userId);
